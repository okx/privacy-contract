/**
 * Browser-compatible cryptographic functions using Web Crypto API
 * This module replaces Node.js crypto module for browser usage
 */
...
/**
 * Gets random bytes using Web Crypto API
 *
...
const aes = {
  gcm: {
    ...
    async encrypt(plaintext: Uint8Array[], key: Uint8Array): Promise<Uint8Array[]> {
      ...
      const totalLength = plaintext.reduce((acc, block) => acc + block.length, 0);
      ...
      // Return [iv + tag, ciphertext] to match original format
      // The original implementation returns [iv+tag, ...data blocks]
      // Since we combined everything, we return as single block
      return [new Uint8Array([...iv, ...tag]), ciphertext];
    },
    ...
    async decrypt(ciphertext: Uint8Array[], key: Uint8Array): Promise<Uint8Array[]> {
      ...
      // Combine encrypted data (all blocks except the first one which contains iv+tag)
      const totalLength = encryptedData.reduce((acc, block) => acc + block.length, 0);
      const combinedEncrypted = new Uint8Array(totalLength + tag.length);
      let offset = 0;
      for (const block of encryptedData) {
        combinedEncrypted.set(block, offset);
        offset += block.length;
      }
      combinedEncrypted.set(tag, offset);
      ...
      // Return as single block (matching original format where multiple blocks are returned)
  ...
  ctr: {
    ...
    async encrypt(plaintext: Uint8Array[], key: Uint8Array): Promise<Uint8Array[]> {
      ...
      // Return [iv, ...encrypted blocks] to match original format
      // Since we combined everything, we return as single block
    ...
    async decrypt(ciphertext: Uint8Array[], key: Uint8Array): Promise<Uint8Array[]> {
      ...
      // Return as single block (matching original format)
      return [new Uint8Array(decrypted)];
    },
  },
};
