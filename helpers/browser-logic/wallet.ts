...
import { randomBytes } from '../browser/crypto-browser';
...
class Wallet {
      ...
      transactionReceipt.logs.map(async (log) => {
        ...
        if (log.address === contract.address) {
          ...
          if (parsedLog.name === 'Shield') {
            ...
            await Promise.all(
              args.shieldCiphertext.map(async (shieldCiphertext, index) => {
                // Try to decrypt (decryptShield is async in browser version)
  ...
  async getUnspentNotes(merkletree: MerkleTree, token: TokenData): Promise<Note[]> {
    ...
    // Get note nullifiers as hex (pass viewingKey as parameter)
    const noteNullifiers = await Promise.all(
      this.notes.map(async (note, index) =>
        arrayToHexString(await note.getNullifier(this.viewingKey, index), false),
      ),
    );
    ...
    const seenNullifiers = merkletree.nullifiers.map((nullifier: Uint8Array) =>
  ...
  async getTestTransactionInputs(
    ...
    // Get output notes (Note constructor no longer needs keys)
    const outputNotes: (Note | UnshieldNote)[] = outputNoteValues.map(
      (value) => new Note(value, randomBytes(16), token, ''),
    );
